...
const Logic = (navigation) => {
  ...
  const navigateToDiagnosis = async (diagnosis) => {
    try {
      ...
      const response = await $securedAxios.get(
        `/Diagnosis/patient/${userId}/diagnosis/${diagnosis.id}`
      );
      ...
      if (
        Array.isArray(fullDiagnosisData.Xray) &&
        fullDiagnosisData.Xray.length > 0
      ) {
      ...
      if (
        Array.isArray(fullDiagnosisData.labResults) &&
        fullDiagnosisData.labResults.length > 0
      ) {
      ...
      if (
        fullDiagnosisData.prescription &&
        Object.keys(fullDiagnosisData.prescription).length > 0
      ) {
        attachments.push({
          id: "prescription-1",
          type: "Prescription",
      ...
      navigation.navigate("Diagnos", {
        ...
        diagnosis: {
          ...
          doctorImage: `${BASE_URL}/${diagnosis.doctorImage}`,
  ...
  const navigateWithBasicData = (diagnosis) => {
    ...
    navigation.navigate("Diagnos", {
      ...
      diagnosis: {
        ...
        specialization: diagnosis.specialization || "General Practice",
        image:
          `${BASE_URL}/${diagnosis.doctorImage}` ||
  ...
  const fetchAllDiagnoses = async () => {
    try {
      ...
      const allDiagnoses = diagnosesArray.map((diagnosis) => ({
        ...
        hasLabResults:
          Array.isArray(diagnosis.labResults) &&
      ...
      console.log("Recent diagnoses count:", recentDiagnoses);
  ...
  const getImageSource = (item) => {
    ...
    if (
      item.type === "X-ray" ||
      (item.name && item.name.includes("X-ray")) ||
      (item.description && item.description.includes("X-ray"))
    ) {
